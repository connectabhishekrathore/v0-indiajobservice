...
import { getVacancyById, updateVacancy } from '@/lib/vacancy'
...
export default function EditVacancyPage() {
  ...
  const [formData, setFormData] = useState<Partial<Vacancy> | null>(null)
  ...
  useEffect(() => {
    const loadVacancy = async () => {
      ...
      if (result.success) {
        setFormData(result.vacancy)
      } else {
        setError(result.error || 'Vacancy not found')
      }
      setLoading(false)
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Update vacancy
      const result = await updateVacancy(vacancyId, session.user.id, {
        ...formData,
      ...
      <main className="min-h-screen bg-background">
        <div className="flex items-center justify-center h-screen">
          <div className="text-center">
            ...
            <p className="text-foreground/60">Loading vacancy...</p>
          </div>
        </div>
      </main>
    )
  ...
  if (!formData) {
    return (
      <main className="min-h-screen bg-background">
        <div className="flex items-center justify-center h-screen">
          <div className="text-center">
            <p className="text-destructive mb-4">{error || 'Vacancy not found'}</p>
    ...
    <main className="min-h-screen bg-background">
      <header className="bg-foreground/5 border-b border-foreground/10">
        <div className="max-w-4xl mx-auto px-4 py-6">
          <h1 className="text-3xl font-bold text-foreground">Edit Vacancy</h1>
        </div>
      </header>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label htmlFor="pdf" className="block text-sm font-medium text-foreground mb-2">
                  Upload Updated Vacancy PDF (optional)
                </label>
                <input
                  id="pdf"
                  type="file"
